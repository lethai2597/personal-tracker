...
export function ResetPasswordPage({
  ...
  async function submit(event: React.FormEvent) {
    ...
    setMessage(response.ok ? t.submitLogin : await response.text());
  }
  ...
  return (
    <AuthShell locale={initialLocale}>
      <form onSubmit={submit} className="w-full max-w-md rounded-[var(--radius-card)] bg-surface-sunken p-6">
        ...
        <button className="mt-6 w-full rounded-full bg-btn px-4 py-3 text-sm font-semibold text-btn-ink">{t.resetTitle}</button>
